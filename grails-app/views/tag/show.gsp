	...
	<body>
		...
		<div id="show-tag" class="content scaffold-show" role="main">
			...
			<ol class="property-list tag">
				...
				<g:if test="${tagInstance?.file}">
				<li class="fieldcontain">
					<span id="file-label" class="property-label"><g:message code="tag.file.label" default="File" /></span>
					
						<g:each in="${tagInstance.file}" var="f">
						<span class="property-value" aria-labelledby="file-label"><g:link controller="availableFile" action="show" id="${f.id}">${f?.encodeAsHTML()}</g:link></span>
						</g:each>
